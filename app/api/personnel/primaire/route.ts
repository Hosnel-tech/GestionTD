...
type ExcelRow = {
  ...
  CENTRE?: string | number | null; // From sheet name
};
...
export async function POST(req: NextRequest) {
  try {
    ...
    let inserted = 0;
    const now = new Date();
    await pool.query("DELETE FROM primaire");
    ...
    for (const row of rows) {
      ...
      const [existingRows] = await pool.query(
        `SELECT id FROM primaire WHERE ifu = ?`,
        [ifu]
      );

      if (Array.isArray(existingRows) && existingRows.length > 0) {
        console.log(`Skipping duplicate ROW:`, ifu);
        continue; // Skip insert if IFU already exists
      }

      const id = `prim_${uuidv4()}`;

      await pool.query(
        `INSERT INTO primaire 
         (id, name, type, ifu, forfait, account_number, bank, centre, created_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    ...
    return NextResponse.json({
      message: `${inserted} enseignants primaires importés.`,
    });
  } catch (err) {
