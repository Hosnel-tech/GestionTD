...
export async function POST(req: NextRequest) {
  try {
    ...
    for (const row of rows) {
      const name = typeof row["Nom et prénoms"] === "string"
      ...
      const ifu = typeof ifuRaw === "string"
        ? ifuRaw.trim()
        : String(ifuRaw ?? "").trim();

      const accountNumber = typeof row["N° de Compte"] === "string"
        ? row["N° de Compte"].trim()
        : String(row["N° de Compte"] ?? "").trim();

      const bank = typeof row["Banques"] === "string"
        ? row["Banques"].trim()
        : String(row["Banques"] ?? "").trim();

      const school = typeof row["Centre"] === "string"
        ? row["Centre"].trim()
        : String(row["Centre"] ?? "").trim();

      const subjects = type.toLowerCase() === "enseignant"
      ...
      const [existing] = await pool.query("SELECT id FROM personnel WHERE ifu = ?", [ifu]);
      ...
      await pool.query(
        `INSERT INTO personnel 
         ...
         VALUES (?, ?, ?, '', ?, '', ?, ?, ?, '', ?, ?)`,
        [id, type, name, school, subjects, ifu, accountNumber, bank, createdAt]
      );
